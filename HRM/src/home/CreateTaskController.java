...
public class CreateTaskController implements Initializable{
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		crt_lbTitleFrom.setText(Main.userLogin.getUsername());
	...
	/**
	 *  Phương thức @popupOverAddUser được sử dụng để hiển thị popup thêm nhân sự vào task
	 *  Phương thức này được load lên từ @PopOverAddStaff 
	 *  Dữ liệu sau khi được chọn sẽ gán cho ArrayList @selectedItems
	 */
	...
	/**
	 * Phương thức @createTask được sử dụng để tạo Task Item cho nhân viên
	 */
	private void createTask() {
		...
		ResultSet rs = dap.insertTask(crt_txtTitle.getText(), Date.valueOf(crt_PickStartDate.getValue()) 		,formatTime(crt_PickHoursStart.getText(), crt_PickMinutesStart.getText()),Date.valueOf(crt_PickFinishDate.getValue()), 		formatTime(crt_PickHoursFinish.getText(), crt_PickMinutesFinish.getText()) , crt_TextArea.getText(), 		Main.userLogin.getId());
	...
	/**
	 * Phương thức @checkValidation được sử dụng để kiểm tra các trường trong CreateTask đã được nhập 
	 * 
	 */
	private boolean checkValidation() {
		...
		if(crt_PickFinishDate.getValue().isAfter(LocalDate.now())) {
