...
public class StaffDAOImpl implements StaffListDAO {
    ...
    public void updateStaff(StaffDTO staff) {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            pstmt.setString(7,staff.getAvatarPath());
            pstmt.setLong(8, staff.getId());
            pstmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public boolean isUsernameExist(String username) {
        String query = "SELECT COUNT(*) FROM Staff WHERE user_name = ?";
